...
const AddProduct = ({ categories }) => {
  ...
  // Inertia form — drives every field that actually gets submitted to the server.
  const { data, setData, post, processing, errors, reset, clearErrors, transform } = useForm({
    ...
    featured: false,
    status: "active",
    variants: [], // [{ size, color, stock_quantity }]
    images: [],   // File[]
  });
  ...
  useEffect(() => {
    ...
    // Spinner keyframes for the submit button loader icon
  ...
  // ─── Client-side validation (fast feedback; server re-validates on submit) ─
  const clientErrors = {};
  ...
  // Server errors (from the last failed submit) take precedence once present.
                  ...
                  <div style={{ padding: "1.5rem", display: "flex", flexDirection: "column", gap: "1rem" }}>
                    ...
                    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "0.75rem" }}>
                      <div>
                        ...
                        <input
                          id="sku" type="text" value={data.sku}
                      ...
                      <div>
                        ...
                        <input
                          id="stock_quantity" type="number" min="0"
                          value={data.stock_quantity} onChange={handleChange("stock_quantity")}
                          placeholder="0"
                          style={inputStyle}
                          onFocus={(e) => (e.target.style.borderColor = tokens.foreground)}
                          onBlur={(e) => (e.target.style.borderColor = tokens.border)}
                      ...
                      <label style={{ display: "flex", alignItems: "center", gap: "0.5rem", cursor: "pointer", fontSize: "0.875rem", color: tokens.foreground }}>
                        <div
                          ...
                          style={{
                            ...
                            border: data.featured ? `1px solid ${tokens.foreground}` : `1px solid ${tokens.border}`,
                            backgroundColor: data.featured ? tokens.foreground : "transparent",
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: "1.5rem" }}>
                ...
                <div style={{ ...cardStyle, overflow: "hidden" }}>
                  ...
                  <div style={{ padding: "1.5rem" }}>
                    ...
                    {errors.images && (
                      <p style={{ fontSize: "0.75rem", color: tokens.destructive, margin: "0 0 1rem" }}>{errors.images}</p>
                    )}
